...
def main():
    ...
    # --- Initial Training Section (One-time, or major updates) ---
    # To train your model, you need a dataset prepared in YOLO format.
    # This involves:
    #    - An image folder (e.g., 'datasets/my_vehicles/images/train')
    #    - A labels folder (e.g., 'datasets/my_vehicles/labels/train') with .txt files corresponding to images
    #    - A data.yaml file (e.g., 'datasets/my_vehicles/data.yaml') specifying paths, class names, etc.

    # Example data.yaml content:
    # # Path to dataset root directory
    # path: ../datasets/my_vehicles
    ...
    # Prepare your data.yaml file and dataset before running training.
    # For instance, if your data.yaml is in `models/vehicle_data.yaml`:
    vehicle_initial_data_config = 'models/vehicle_data.yaml'
    license_plate_initial_data_config = 'models/license_plate_data.yaml' # If training separate LP model

    # To perform an initial, large-scale training for the vehicle detection model:
    if os.path.exists(vehicle_initial_data_config):
        logger.info("Attempting to perform initial training for vehicle model...")
        trained_vehicle_model = detector.train_model(
            data_config_path=vehicle_initial_data_config,
            epochs=100, # Adjust as needed (can be high for initial training)
            batch_size=8, # Adjust based on GPU memory
            img_size=640,
            model_type='vehicle',
            save_dir='runs/initial_train_vehicles'
        )
        if trained_vehicle_model:
            logger.info("Initial vehicle model training completed. New model is loaded for detection.")
            detector.validate_model(
                model_path=str(trained_vehicle_model.pt),
                data_config_path=vehicle_initial_data_config,
                model_type='vehicle'
            )
    else:
        logger.warning(f"Vehicle data config not found at {vehicle_initial_data_config}. Skipping initial vehicle model training.")

    # To perform an initial, large-scale training for a separate license plate detection model:
    # if os.path.exists(license_plate_initial_data_config):
    #     logger.info("Attempting to perform initial training for license plate model...")
    #     trained_license_plate_model = detector.train_model(
    #         data_config_path=license_plate_initial_data_config,
    #         epochs=150, # License plates might need more epochs
    #         batch_size=16,
    #         img_size=320, # License plates are smaller, can use smaller img_size
    #         model_type='license_plate',
    #         save_dir='runs/initial_train_license_plates'
    #     )
    #     if trained_license_plate_model:
    #         logger.info("Initial license plate model training completed. New model is loaded for detection.")
    #         detector.validate_model(
    #             model_path=str(trained_license_plate_model.pt),
    #             data_config_path=license_plate_initial_data_config,
    #             model_type='license_plate'
    #         )
    # else:
    ...
    if os.path.exists(sample_annotated_data_for_continuous_learning) and os.path.isdir(sample_annotated_data_for_continuous_learning):
        ...
        logger.info("\n--- Starting Continuous Learning Simulation ---")
        
        detector.simulate_new_data_arrival(num_samples=20, base_data_dir=sample_annotated_data_for_continuous_learning)
        detector.simulate_new_data_arrival(num_samples=30, base_data_dir=sample_annotated_data_for_continuous_learning) # Add more data

     
        detector.retraining_threshold = 10
        detector.retraining_interval_hours = 0.5

       
        num_continuous_learning_cycles = 3
        logger.info(f"Running {num_continuous_learning_cycles} continuous learning cycles (simulated).")
        for cycle in range(num_continuous_learning_cycles):
            logger.info(f"\n--- Continuous Learning Cycle {cycle + 1}/{num_continuous_learning_cycles} ---")
            # Simulate more data arriving between cycles
            if cycle < num_continuous_learning_cycles -1:
                 detector.simulate_new_data_arrival(num_samples=random.randint(5, 15), base_data_dir=sample_annotated_data_for_continuous_learning)

            detector.initiate_continuous_learning(
                vehicle_data_config_template=vehicle_initial_data_config,
                training_epochs=5,  # Fewer epochs for incremental updates
                training_batch_size=8,
                check_interval_seconds=1 # Very short interval for demo purposes
            )
          
        logger.info("\n--- Finished Continuous Learning Simulation ---")

    else:
        logger.warning(f"Sample annotated data directory not found at {sample_annotated_data_for_continuous_learning}. Continuous learning simulation skipped.")
        logger.info("To run continuous learning, please prepare a directory with annotated data and update the `sample_annotated_data_for_continuous_learning` variable.")
